...
print('FPGAs configured successfully. Sending Synapse packets now....')
try:
	SynapseArray = rx.xmlParseBeforePublishing(synXmlFile)
#error handling

	for i in range(len(SynapseArray)):
		packet = SynapseArray[i]
		print(packet)
		TCPclient(host,port,packet)
		time.sleep(0.5)
		print("sent paket {} out of {}".format(i,len(SynapseArray)))
except:
	print("cannot read")

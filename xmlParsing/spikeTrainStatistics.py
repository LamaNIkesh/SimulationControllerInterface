...
def raster(event_times_list, **kwargs):
    ...
    for ith, trial in enumerate(event_times_list):
        plt.vlines(trial, ith + .2, ith + .3, **kwargs)
...
if __name__ == "__main__":
	...
	'''
	spiketrain = []
	SpikeFile_FPGA = 'resultscsv.txt'
	try:
		with open(SpikeFile_FPGA) as f:
			lines=f.readlines()
			for line in lines:
	...
	for ith, trial in enumerate(spiketrain):
		ax1.vlines(trial, ith + .5, ith + 0.8)
	ax1.set_title('Raster Plot')
	#ax1.set_xlabel('Time(ms)')
	ax1.set_ylabel('Neuron #')

	#PSTH plot
	PSTH_binCount,noOfbins,binSize = PSTH_multi(spiketrain, binSize = 2 , totalTime = 1000)
	PSTH_realTimeAxis = PSTH_realTimeAxis(PSTH_binCount, binSize)	
	x = np.arange(np.size(PSTH_realTimeAxis))
	#plt.figure(figsize = (20,10))
	#ax2.set_xlim([-5,4000])
	ax2.bar(x,PSTH_realTimeAxis, width = binSize, color = 'red',label = '# of spikes/bin')
